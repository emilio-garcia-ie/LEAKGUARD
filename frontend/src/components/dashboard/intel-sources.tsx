...
const SOURCES = [
  ...
  { name: "Shodan", icon: Radio, statusKey: "source_status_configured" as const, url: "https://shodan.io", desc: "Exposed devices", iconColor: "text-cyan-400" },
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-800">
        ...
        <span className="ml-auto text-slate-500 text-xs hidden sm:block">{t.intel_sources_subtitle}</span>
      </div>
      <div className="grid grid-cols-3 sm:grid-cols-5 xl:grid-cols-9 gap-0 divide-x divide-y divide-slate-800/50">
        {SOURCES.map((src) => {
          ...
          return (
            <a
              ...
              className="flex flex-col items-center gap-1.5 p-3 hover:bg-slate-800/60 transition-colors group text-center"
            >
              ...
              <span className="text-slate-500 text-[10px] leading-tight">{src.desc}</span>
              ...
              {src.url !== "#" && (
                <ExternalLink className="w-2.5 h-2.5 text-slate-600 group-hover:text-slate-400 transition-colors" />
              )}
            </a>
          );
        })}
