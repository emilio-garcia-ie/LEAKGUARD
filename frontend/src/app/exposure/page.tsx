  ...
  return (
    <ProtectedRoute>
      <AppShell>
        ...
        <div className="flex gap-1.5 mb-6 p-1 bg-slate-950/60 border border-slate-800 rounded-xl w-fit">
          <button
          ...
          <CardContent className="pt-6 space-y-4">
            <div className="flex gap-2 flex-col sm:flex-row">
              ...
              <div className="flex gap-2">
                {searchTarget === "breaches" && (
                  <div className="relative">
                    <select
                      ...
                      className="appearance-none pl-3 pr-8 py-2 rounded-lg border border-slate-700 bg-slate-900 text-slate-200 text-sm h-full focus:outline-none focus:border-cyan-500 hover:border-slate-600 transition-colors cursor-pointer"
                    >
                ...
                <Button onClick={runScan} disabled={loading} className="gap-2 bg-cyan-600 hover:bg-cyan-500 text-white font-semibold whitespace-nowrap">
            ...
            {searchTarget === "breaches" && (
              <div className="flex items-center gap-2 text-xs">
                <span className="text-slate-500">Modo de búsqueda:</span>
                <span className={cn(
        ...
        {result && (
          <>
            ...
            {searchTarget === "breaches" && (
              <>
                ...
                {result.sourcesChecked && (
                  <Card className="mb-6 border-slate-800 bg-slate-950/20 overflow-hidden">
                    <CardHeader className="border-b border-slate-800/80 bg-slate-900/10 py-3">
                    ...
                    <CardContent className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {result.sourcesChecked.map((src) => (
                        <a
                          ...
                          className="flex items-center justify-between p-3 rounded-lg border border-slate-800/60 bg-slate-900/30 hover:bg-slate-800/30 hover:border-slate-700 transition-all group"
                        >
            ...
            {searchTarget === "actors" && result.actorProfile && (
              <div className="grid lg:grid-cols-3 gap-6 mb-6">
                ...
                <Card className="lg:col-span-2 overflow-hidden border-purple-900/30">
                  <CardHeader className="bg-gradient-to-r from-purple-950/20 to-indigo-950/20 border-b border-slate-800/80">
                  ...
                  <CardContent className="pt-6 space-y-4">
                    <div className="flex justify-between border-b border-slate-800/60 pb-3 text-sm">
                      <span className="text-slate-400 font-medium">País / Origen:</span>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed font-sans bg-slate-950/40 border border-slate-900 p-3 rounded-lg">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {result.actorProfile.targetSectors.map((sector) => (
                            <Badge key={sector} className="bg-cyan-950/40 text-cyan-400 border border-cyan-800/20 text-[10px]">
                              {sector}
                            </Badge>
                          ))}
                        </div>
                  ...
                  <Card className="border-rose-900/30">
                    <CardContent className="pt-6">
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Puntuación de Riesgo</span>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-2.5 pt-0">
                      {Object.entries(result.actorProfile.externals).map(([dbName, details]) => (
                        <div key={dbName} className="flex flex-col p-2.5 rounded-lg border border-slate-800 bg-slate-900/20 text-xs">
                          ...
                          <span className="text-[10px] text-slate-500 leading-tight font-mono">{details}</span>
                        </div>
                      ))}
                    </CardContent>
                  </Card>
            ...
            <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-slate-900/50 border border-slate-800 p-4 rounded-xl">
              <div>
              ...
              <Button
                ...
                disabled={aiLoading}
                className="gap-2 bg-gradient-to-r from-cyan-600 to-indigo-600 hover:from-cyan-500 hover:to-indigo-500 text-white font-bold text-xs shrink-0 self-end sm:self-center"
              >
            ...
            {aiLoading && (
              <Card className="mb-6 border-cyan-800/40 bg-slate-950/30 animate-pulse">
                <CardContent className="py-8 flex flex-col items-center justify-center gap-3">
                  ...
                  <div className="text-xs text-slate-400 font-medium">Gemini está analizando los detalles de inteligencia...</div>
                </CardContent>
              </Card>
            )}
            ...
            {aiResponse && (
              <Card className="mb-6 border-cyan-500/30 bg-slate-900/60 shadow-[0_0_20px_rgba(34,211,238,0.05)] overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-cyan-950/30 to-indigo-950/30 border-b border-slate-800 py-3 flex flex-row items-center justify-between">
                  <CardTitle className="text-xs font-bold text-cyan-400 flex items-center gap-2 uppercase tracking-wider">
                    <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
                    Informe de Inteligencia Artificial (Gemini)
                  </CardTitle>
                  <span className="text-[10px] text-slate-500 font-mono">Modo: {aiResponse.model || "Gemini"}</span>
                </CardHeader>
                <CardContent className="pt-4 pb-5 text-sm text-slate-200 leading-relaxed whitespace-pre-wrap font-sans">
                  {aiResponse.answer}
                </CardContent>
              </Card>
            )}
            ...
            <Card className="overflow-x-auto">
              ...
              <CardContent className="p-0">
                <table className="w-full text-xs min-w-[600px]">
                  <thead className="text-slate-500 border-b border-slate-800 bg-slate-900/10">
                    {searchTarget === "actors" ? (
                      <tr>
                    ...
                    {searchTarget === "actors" ? (
                      result.records.map((r, i) => (
                        <tr key={i} className="border-b border-slate-800/40 hover:bg-slate-900/40 transition-colors">
                          <td className="px-4 py-2 font-mono text-slate-400 whitespace-nowrap">{r.date}</td>
                          <td className="px-4 py-2 font-mono text-cyan-300 font-bold truncate">{r.id}</td>
                          <td className="px-4 py-2 text-slate-200">{r.victim}</td>
                          <td className="px-4 py-2 text-slate-400">{r.sector}</td>
                          <td className="px-4 py-2 text-slate-400">{r.country}</td>
                      ...
                      result.records.map((r, i) => (
                        <tr key={i} className="border-b border-slate-800/40 hover:bg-slate-900/40 transition-colors">
                          <td className="px-4 py-2 font-mono text-slate-400 whitespace-nowrap">{String(r.date)}</td>
