...
function parseMarkdown(text: string) {
  ...
  return parts.map((part, index) => {
    if (part.startsWith("**") && part.endsWith("**")) {
      return (
        <strong key={index} className="text-cyan-400 font-extrabold">
          {part.slice(2, -2)}
        </strong>
      );
    }
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-6">
            <Card>
              <CardContent className="pt-6">
                <div className="text-3xl font-bold text-cyan-400">{metrics.verificationRate}%</div>
                <div className="text-sm text-slate-500">{t.ai_verif_rate}</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <div className="text-3xl font-bold text-orange-400">{metrics.falsePositiveRate}%</div>
                <div className="text-sm text-slate-500">{t.ai_false_positive}</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <div className="text-3xl font-bold text-purple-400">{metrics.avgConfidence}%</div>
                <div className="text-sm text-slate-500">{t.ai_confidence}</div>
              </CardContent>
            </Card>
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <Card className="h-full flex flex-col">
              ...
              <CardContent className="space-y-4 flex-1 flex flex-col justify-between">
                <div>
                  <p className="text-sm text-slate-400 mb-4">{t.ai_desc}</p>
                  <Button onClick={runAnalysis} className="w-full" disabled={loading}>
                    {loading ? t.ai_running_btn : t.ai_run_btn}
                  </Button>
                </div>
                {analysis && (
                  <div className="mt-4 p-4 rounded-xl border border-slate-800 bg-slate-950/60 text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">
          ...
          <div className="lg:col-span-2">
            <Card className="h-[520px] flex flex-col overflow-hidden">
              <CardHeader className="border-b border-slate-800/80 bg-slate-900/10">
              ...
              <CardContent className="flex-1 flex flex-col justify-between p-0 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 font-sans">
                        ...
                        <div
                          className={`w-7 h-7 rounded-full flex items-center justify-center text-xs flex-shrink-0 ${
                            msg.sender === "user"
                              ...
                              : "bg-cyan-950/50 text-cyan-300 border border-cyan-800/30"
                          }`}
                        >
                  ...
                  {chatLoading && (
                    <div className="flex gap-3 max-w-[85%]">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs bg-cyan-950/50 text-cyan-300 border border-cyan-800/30 animate-pulse">
                        <Bot className="w-3.5 h-3.5" />
                      </div>
                      <div className="p-3 rounded-2xl text-xs bg-slate-900/60 text-slate-500 border border-slate-800/80 rounded-tl-none animate-pulse">
                ...
                {/* Input block */}
                <form onSubmit={sendChatMessage} className="p-3 border-t border-slate-800/80 bg-slate-900/20 flex gap-2">
                  <input
                    type="text"
                    value={inputMsg}
                    ...
                    className="flex-1 px-3 py-2 text-xs rounded-lg border border-slate-800 bg-slate-950/80 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500/80 disabled:opacity-40 disabled:cursor-not-allowed"
                  />
