...
export default function TermsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#040B14] text-slate-100 overflow-x-hidden relative" style={{ fontFamily: "'Inter', sans-serif" }}>
      <CyberCanvas />
      ...
      <main className="relative z-10 pt-28 pb-20 px-4 max-w-4xl mx-auto" dir={isRtl ? "rtl" : "ltr"}>
        ...
        <div className="p-8 md:p-10 rounded-3xl border border-white/10 bg-slate-900/60 backdrop-blur-md relative overflow-hidden shadow-2xl">
          ...
          <div className={`flex items-center gap-4 mb-6 ${isRtl ? "flex-row-reverse" : ""}`}>
            <div className="w-12 h-12 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center flex-shrink-0 text-cyan-400 shadow-md">
              <Shield className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">{currentTerms.title}</h1>
              <p className="text-slate-400 text-sm mt-1">{currentTerms.subtitle}</p>
            </div>
          </div>

          <div className="border-b border-white/5 pb-4 mb-6">
            <span className="text-xs font-mono text-cyan-400/70 bg-cyan-950/40 px-2.5 py-1 rounded border border-cyan-800/30">
      ...
      {/* Sticky Footer */}
      <footer className="border-t border-white/5 py-6 px-4 text-center relative z-10 bg-[#040B14]/80 backdrop-blur-md">
